...
let _searchGlow=null
function highlightGun(gunW){
  if(_searchGlow){
    clearInterval(_searchGlow.iv)
    sc.remove(_searchGlow.sp);_searchGlow.sp.material.dispose();_searchGlow=null
  }
  const wp=new T.Vector3();gunW.getWorldPosition(wp)
  const mat=new T.SpriteMaterial({color:'#33ee55',transparent:true,opacity:1,depthWrite:false,depthTest:false,blending:T.AdditiveBlending})
  const sp=new T.Sprite(mat)
  sp.scale.set(.7,.7,1)
  sp.position.copy(wp);sp.position.y+=1.25
  sc.add(sp)
  _searchGlow={sp,mat}
  let n=0
  _searchGlow.iv=setInterval(()=>{
    n++
    mat.opacity=n%2===1?1:.12
    if(n>=16){
      clearInterval(_searchGlow.iv)
      sc.remove(sp);mat.dispose()
      if(_searchGlow&&_searchGlow.sp===sp)_searchGlow=null
    }
  },350)
}
// ── STATISTICS ──
