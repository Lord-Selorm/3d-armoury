...
ctrl.minPolarAngle=.1;ctrl.maxPolarAngle=Math.PI/2.05
ctrl.minDistance=1.5;ctrl.maxDistance=25
ctrl.update()

// ── MINIMAP ──
const mapRdr=new T.WebGLRenderer({canvas:document.getElementById('map'),antialias:true,alpha:true})
mapRdr.setSize(120,80);mapRdr.setPixelRatio(Math.min(devicePixelRatio,2))
mapRdr.toneMapping=T.ACESFilmicToneMapping;mapRdr.toneMappingExposure=.7
const mapSc=new T.Scene()
const mapCam=new T.OrthographicCamera(-6.5,6.5,4.5,-4.5,.1,25)
mapCam.position.set(0,12,0);mapCam.lookAt(0,0,-3.5)
const mapFloor=new T.Mesh(new T.PlaneGeometry(16,12),new T.MeshBasicMaterial({color:'#181a22',transparent:true,opacity:.5}))
mapFloor.rotation.x=-Math.PI/2;mapFloor.position.set(0,0,-3.5);mapSc.add(mapFloor)
const mg=new T.GridHelper(16,8,'#2a2a36','#22222e');mg.position.set(0,0,-3.5);mapSc.add(mg)
const mFont=(c,y,label)=>{
  const cv=document.createElement('canvas');cv.width=100;cv.height=20
  const ctx=cv.getContext('2d');ctx.fillStyle=c;ctx.font='bold 11px monospace'
  ctx.textAlign='center';ctx.fillText(label,50,14)
  const tx=new T.CanvasTexture(cv);tx.minFilter=T.LinearFilter
  const m=new T.SpriteMaterial({map:tx,transparent:true,opacity:.5,depthTest:false})
  const s=new T.Sprite(m);s.position.set(0,0,y);s.scale.set(2,.4,1);mapSc.add(s)
}
mFont('#d4b898',0,'R.D.F');mFont('#b8c8d8',-3.5,'81 SIG');mFont('#c8b8a8',-7.5,'SOUTH')
let _mapBoxes=[]
...
for(let i=0;i<dCount;i++){
  dPos[i*3]=(Math.random()-.5)*16
  dPos[i*3+1]=Math.random()*4.5+.5
  dPos[i*3+2]=(Math.random()-.5)*14-3.5
}
dGeo.setAttribute('position',new T.BufferAttribute(dPos,3))
const dMat=new T.PointsMaterial({color:'#c8b898',size:.06,transparent:true,opacity:.5,blending:T.AdditiveBlending,depthWrite:false})
const dust=new T.Points(dGeo,dMat);sc.add(dust)
// ── FLICKERING CEILING LIGHTS ──
const flickerLights=[]
for(let i=0;i<6;i++){
  const l=new T.PointLight(0xffeecc,4+Math.random()*2,14)
  l.position.set((Math.random()-.5)*10,4.2,(Math.random()-.5)*8-3.5)
  l.userData={baseInt:l.intensity,phase:Math.random()*100,spd:.3+Math.random()*.5}
  sc.add(l),flickerLights.push(l)
}
// ── GROUND REFLECTION (fake cubemap via subtle glossy plane) ──
const refMat=new T.MeshPhysicalMaterial({
  color:'#393845',roughness:.25,metalness:.9,
  transparent:true,opacity:.12,depthWrite:false,envMapIntensity:.5
})
const refPlane=new T.Mesh(new T.PlaneGeometry(flrW,flrD),refMat)
refPlane.rotation.x=-Math.PI/2;refPlane.position.y=.005;sc.add(refPlane)
...
const wd=new T.MeshPhysicalMaterial({map:wdTex,color:0x8a6a4a,roughness:.85,metalness:0,clearcoat:.06,clearcoatRoughness:.4})
const wl=new T.MeshPhysicalMaterial({map:wlTex,color:0xa08060,roughness:.78,metalness:0,clearcoat:.05,clearcoatRoughness:.5})
const wk=new T.MeshPhysicalMaterial({map:wkTex,color:0x6a4e2e,roughness:.9,metalness:0})
...
const secs=[
  ...
  {name:'Southern Command',color:'#c8b8a8',racks:[
    ...
    {a1:20,a2:0,gun:{a1:[{type:'sr25',count:7},{type:'negev',count:7},{type:'m60',count:6}]}},{a1:9,a2:0,gun:{a1:[{type:'mg42',count:3},{type:'c90',count:3},{type:'rpg',count:3}]}},{a1:10,a2:10,gun:'m16'}
  ]}
]
...
let _loadingState=false, _psave=()=>{}, _pupdate=()=>{}
...
function drawMap(){
  _mapBoxes.forEach(mb=>{
    const rd=mb.userData
    if(!rd)return
    const tot=rd.a1+rd.a2,rem=(rd.a1Rem||0)+(rd.a2Rem||0)
    const r=rem/tot,missing=tot-rem
    mb.material.color.set(r<.25?'#e06840':r<1?'#d0a840':'#30b860')
    mb.material.opacity=missing?1:.4
    mb.scale.z=.08+missing*.12
    if(missing>0&&missing===tot)mb.material.color.set('#ee3333')
  })
}
function resetAll(){
...
const loadAll=Promise.all([loadGLB('m16.glb'),loadGLB('ak47.glb'),loadGLB('uzi.glb'),loadGLB('sr25.glb'),loadGLB('negev.glb'),loadGLB('m60.glb'),loadGLB('mg42.glb'),loadGLB('c90.glb'),loadGLB('rpg.glb'),loadFBX('m4a1_oga/M4A1.fbx'),loadFBX('g3_model/Gun.fbx'),loadHDR('warehouse.hdr')])
...
Promise.race([loadAll,loadTimeout]).then(v=>{
  ...
  if(refPlane.material)refPlane.material.envMap=envMap
  ...
  const m4a1t=prep(isFbx?m4a1g:m4a1g.scene)
  const g3t=prep(g3g)
  const uzit=prep(uzig.scene);uzit.scale.multiplyScalar(.55)
  const sr25t=prep(sr25g.scene);sr25t.scale.multiplyScalar(.85)
  const negevt=prep(negevg.scene)
  const m60t=prep(m60g.scene)
  const mg42t=prep(mg42g.scene)
  const c90t=prep(c90g.scene)
  const rpgt=prep(rpgg.scene)
  ...
  function placeGuns(rack,template,n,side,slant,rackW,tint,offset=0,total=n,gunType){
    ...
    for(let i=0;i<n;i++){
      ...
      const iMat=new T.MeshStandardMaterial({color:'#22cc44',emissive:'#33ee55',emissiveIntensity:.8,roughness:.2,metalness:0})
  ...
  rows.forEach((row,ri)=>{
    ...
    row.racks.forEach((r,idx)=>{
      ...
      const lbl=mkLabel(`${rackId}  ${fmtGun(gunA1)}`,'label-rack','#e8d8c0')
      ...
      const slant=Math.atan2(.62,1.76)
      ...
      const gMat=new T.MeshStandardMaterial({
        color:'#444',emissive:'#000',emissiveIntensity:3,
        transparent:true,opacity:0,
        roughness:.3,metalness:.2
      })
      const gBar=new T.Mesh(new T.BoxGeometry(rackW-AB*1.6,.035,.035),gMat)
      gBar.position.set(0,1.88,0)
      rack.add(gBar)
      rack.userData.glowEl=gBar

      const mBox=new T.Mesh(new T.BoxGeometry(rackW,.08,.08),new T.MeshBasicMaterial({color:'#30b860',transparent:true,opacity:.75}))
      mBox.position.set(rx+rackW/2,.06,rz)
      mBox.userData=rack.userData
      mapSc.add(mBox)
      _mapBoxes.push(mBox)
      rack.userData.mBox=mBox
...
function moveCam(){
  if(_focus||!Object.values(keys).some(v=>v))return
  const spd=.3
  ...
  if(_walkMode){
    ...
    if(mv.length()>.01){mv.normalize().multiplyScalar(spd);cam.position.add(mv);ctrl.target.add(mv);ctrl.target.y=1.4}
    return
  }
...
function checkoutGun(gunW){
  ...
  if(gunW.userData.indEl){const m=gunW.userData.indEl.material;m.color.set('#cc2222');m.emissive.set('#cc2222')}
  updateRackLabel(rd)
  if(!_loadingState){
...
function restoreGun(rack){
  ...
  if(gunW.userData.indEl){const m=gunW.userData.indEl.material;m.color.set('#22cc44');m.emissive.set('#22cc44')}
  updateRackLabel(rd)
  if(!_loadingState){
    const wp=new T.Vector3();gunW.getWorldPosition(wp)
    burst(wp,8,'#44ff88')
...
function updateRackLabel(rd){
  ...
  if(rd.glowEl){
    const tot=a1+a2
    if(tot){const n=(a1R+a2R)/tot
      if(n<1){const m=rd.glowEl.material;m.opacity=.85;m.color.set(n<.25?'#ee5533':n<.5?'#ee9922':'#ddcc44');m.emissive.set(n<.25?'#ee4400':n<.5?'#ee8800':'#ddbb33')}
...
function toggleWalk(){
  ...
  if(_walkMode){ctrl.target.set(cam.position.x,1.4,cam.position.z-2);ctrl.update()}
}
// ── PRESET VIEWS ──
const _presets={
...
document.getElementById('map').addEventListener('click',e=>{
  ...
  if(!_qrRack||_qrRack==='ARMOURY'){
    updateQR('RDF · 81 SIG · SC')
  }else if(_qrRack==='RDF · 81 SIG · SC'){
    updateQR(null)
  }else{
    updateQR('ARMOURY')
  }
...
function tick(){
  ...
  for(let i=0;i<dCount;i++){
    dp[i*3+1]-=.001;dp[i*3]+=Math.sin(Date.now()*.0003+i)*.0003
    if(dp[i*3+1]<0){dp[i*3+1]=4.5+Math.random()*.5;dp[i*3+2]=(Math.random()-.5)*14-3.5}
  }
  dust.geometry.attributes.position.needsUpdate=true
  const t=Date.now()*.001,pt=Math.sin(t*.5)
  flickerLights.forEach(l=>{l.intensity=l.userData.baseInt*(.7+.3*Math.sin(t*l.userData.spd+l.userData.phase))})
  sc.children.forEach(c=>{
    if(!c.userData?.isRack)return
    const g=c.userData.glowEl
    if(g&&g.material.opacity>.01)g.material.emissiveIntensity=2+pt
  })
  if(_walkMode&&cam.position.y<.2)cam.position.y=.2
  if(_inspect)_inspect.gRdr.render(_inspect.gSc,_inspect.gCam)
  if(!_animCam()&&!_inspect)ctrl.update()
  rdr.render(sc,cam);ldr.render(sc,cam);mapRdr.render(mapSc,mapCam)
}
tick()
